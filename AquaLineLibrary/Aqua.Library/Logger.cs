...
namespace Aqua.Library
{
    ...
    public class Logger
    {
        ...
        public bool initialize(bool debug, string filename)
        {
            ...
            return initialize(debug, binDir + filename, 1024000, 10);
        }
        ...
        private String convertToLogMsg(int level, String msg, object[] vars)
        {
            ...
            if (vars != null)
            {
                ...
                {
                    if (vars[i].ToString().Contains("%"))
                        builder.Replace("{" + i + "}", vars[i].ToString());
                    else
                        builder.Append(vars[i].ToString());
                }
            }
